            ...
            <div class="hidden items-center gap-3 md:flex">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.outside="open = false"
                        ...
                        <a href="?lang=en" @click="open = false" class="flex items-center gap-2 px-4 py-2 text-sm transition-colors {{ $currentLocale === 'en' ? 'font-semibold text-emerald-700 bg-emerald-50 dark:text-emerald-300 dark:bg-emerald-900/20' : 'text-zinc-700 hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-700' }}">
                            @if($currentLocale === 'en')<svg class="size-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>@endif
                            English
                        </a>
                        <a href="?lang=ar" @click="open = false" class="flex items-center gap-2 px-4 py-2 text-sm transition-colors {{ $currentLocale === 'ar' ? 'font-semibold text-emerald-700 bg-emerald-50 dark:text-emerald-300 dark:bg-emerald-900/20' : 'text-zinc-700 hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-700' }}">
                            @if($currentLocale === 'ar')<svg class="size-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>@endif
                            العربية
                        </a>
                    </div>
                </div>
                ...
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-lg bg-emerald-600 px-5 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-700">{{ __('Dashboard') }}</a>
                @else
                    @if(Route::has('login'))
                    ...
                    @if(Route::has('register'))
                        <a href="{{ route('register') }}" class="rounded-lg bg-emerald-600 px-5 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-700">{{ __('Get Started Free') }}</a>
                    @endif
                @endauth
            </div>
        ...
        <div x-show="mobileOpen" x-cloak
            ...
            <div class="mt-4 border-t border-zinc-200 pt-4 dark:border-zinc-800">
                ...
                <div class="flex gap-1">
                    <a href="?lang=en" @click="mobileOpen = false" class="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm {{ $currentLocale === 'en' ? 'font-semibold text-emerald-700 bg-emerald-50 dark:text-emerald-300 dark:bg-emerald-900/20' : 'text-zinc-600 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-800' }}">
                        @if($currentLocale === 'en')<svg class="size-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>@endif
                        English
                    </a>
                    <a href="?lang=ar" @click="mobileOpen = false" class="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm {{ $currentLocale === 'ar' ? 'font-semibold text-emerald-700 bg-emerald-50 dark:text-emerald-300 dark:bg-emerald-900/20' : 'text-zinc-600 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-800' }}">
                        @if($currentLocale === 'ar')<svg class="size-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>@endif
                        العربية
                    </a>
                </div>
            ...
            <div class="mt-4 border-t border-zinc-200 pt-4 dark:border-zinc-800">
                @auth
                    <a href="{{ route('dashboard') }}" class="block w-full rounded-lg bg-emerald-600 px-4 py-2.5 text-center text-sm font-medium text-white transition-colors hover:bg-emerald-700">{{ __('Dashboard') }}</a>
                @else
                    <div class="flex flex-col gap-2">
                        ...
                        @endif
                        @if(Route::has('register'))
                            <a href="{{ route('register') }}" class="block rounded-lg bg-emerald-600 px-4 py-2.5 text-center text-sm font-medium text-white transition-colors hover:bg-emerald-700">{{ __('Get Started Free') }}</a>
                        @endif
                    </div>
                @endauth
            </div>
