...
@endpush
    ...
    {{-- Hero --}}
    <section class="border-b border-zinc-200 bg-zinc-50 py-16 dark:border-zinc-200 dark:bg-white">
        <div class="mx-auto max-w-6xl px-6 text-center">
            <h1 class="text-4xl font-bold tracking-tight sm:text-5xl">{{ __('OT1-Pro Blog') }}</h1>
            <p class="mx-auto mt-4 max-w-2xl text-lg text-zinc-600 dark:text-zinc-600">
        ...
        <div class="mx-auto max-w-6xl px-6">
            ...
            @else
                <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <article class="group flex flex-col rounded-2xl border border-zinc-200 bg-white overflow-hidden transition-all hover:shadow-md dark:border-zinc-200 dark:bg-white">
                        {{-- Category badge --}}
                        <div class="p-6 pb-0">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            <h2 class="text-lg font-semibold leading-snug group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                <a href="{{ route('blog.show', $post->slug) }}">{{ $post->title }}</a>
                            </h2>
                            <p class="mt-3 flex-1 text-sm text-zinc-600 dark:text-zinc-600 line-clamp-3">{{ $post->excerpt }}</p>

                            <div class="mt-5 flex items-center justify-between text-xs text-zinc-600">
                        ...
                        <div class="border-t border-zinc-100 px-6 py-3 dark:border-zinc-200">
                            <a href="{{ route('blog.show', $post->slug) }}" class="flex items-center gap-1 text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                                {{ __('Read article') }}
    ...
    <section class="border-t border-zinc-200 bg-zinc-50 py-16 dark:border-zinc-200 dark:bg-white">
        <div class="mx-auto max-w-2xl px-6 text-center">
            ...
            <p class="mt-3 text-zinc-600 dark:text-zinc-600">{{ __('Connect WhatsApp, Instagram, Facebook & Telegram in one inbox with AI that sells for you.') }}</p>
            <a href="{{ route('register') }}" class="mt-6 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-8 py-3.5 font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all hover:bg-indigo-700">
                {{ __('Get Started Free') }}
            </a>
        </div>
    </section>
