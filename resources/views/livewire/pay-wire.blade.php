<div class="min-h-screen bg-zinc-50 py-16 px-4">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="text-center mb-10">
            ...
            <h1 class="text-3xl font-bold text-zinc-900">{{ __('Wire Transfer Payment') }}</h1>
            <p class="mt-2 text-zinc-500">{{ __('Pay via international bank transfer and we\'ll activate your workspace within 12–24 hours.') }}</p>
        </div>
        ...
        @if($submitted)
            ...
            <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-10 text-center">
                ...
                <h2 class="text-xl font-bold text-zinc-900 mb-2">{{ __("We've received your submission!") }}</h2>
                <p class="text-zinc-500 max-w-sm mx-auto">
                    {{ __("We'll review your receipt and activate your account early — usually within a few hours. Expect a confirmation email at") }} <strong>{{ $email }}</strong>.
                </p>
            ...
            <div class="grid gap-6">
                ...
                <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
                    ...
                    <div class="px-6 py-5 space-y-3">
                        @php
                            $bankDetails = [
                                __('Account Holder') => env('BANK_HOLDER_NAME', 'Omar Mohamed Ali'),
                                __('Bank Name')      => env('BANK_NAME', 'National Bank of Egypt (NBE)'),
                                __('SWIFT / BIC')    => env('BANK_SWIFT', 'NBEGEGCX134'),
                                __('IBAN')           => env('BANK_IBAN', 'EG180003013450818264140000110'),
                            ];
                        @endphp
                        ...
                        <div class="mt-3 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
                            <p class="text-xs text-amber-800">
                                <strong>{{ __('Important:') }}</strong> {{ __('Include') }} <code class="bg-amber-100 px-1 rounded">OT1 Pro SaaS Subscription</code> {{ __('in your transfer notes/memo field to expedite compliance clearance.') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
                    ...
                    <form wire:submit="submit" class="px-6 py-5 space-y-5">
                        ...
                        <flux:select :label="__('Plan you\'re subscribing to')" wire:model="plan">
                            <option value="basic">{{ __('Basic — $8/month') }}</option>
                            <option value="starter">{{ __('Starter — $29/month') }}</option>
                            <option value="pro">{{ __('Pro — $79/month') }}</option>
                        </flux:select>

                        <div class="grid sm:grid-cols-2 gap-4">
                            <flux:input
                                :label="__('Your bank name')"
                                wire:model="bank_name"
                                :placeholder="__('e.g. HSBC, Mashreq, Chase')"
                                required
                            />
                            <flux:input
                                :label="__('Your country')"
                                wire:model="bank_country"
                                :placeholder="__('e.g. UAE, USA, UK')"
                                required
                            />
                        </div>

                        <flux:input
                            :label="__('Transaction reference / TXID (optional)')"
                            wire:model="txid"
                            :placeholder="__('Reference number shown on your bank receipt')"
                        />

                        <div>
                            <label class="block text-sm font-medium text-zinc-700 mb-1.5">
                                {{ __('Attach receipt / transaction screenshot') }} <span class="text-red-500">*</span>
                            </label>
                            <div class="relative">
                                <input
                                    type="file"
                        ...
                        <flux:button type="submit" variant="primary" class="w-full" wire:loading.attr="disabled">
                            <span wire:loading.remove>{{ __('Submit Payment Details') }}</span>
                            <span wire:loading>{{ __('Submitting...') }}</span>
                        </flux:button>

                        <p class="text-xs text-center text-zinc-400">
                            {{ __('SWIFT transfers take 2–5 business days to clear. We\'ll provision your account as soon as we see your receipt — usually within a few hours.') }}
                        </p>
                    </form>
                </div>
