<div class="p-6 max-w-5xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('super-admin.blog.index') }}" wire:navigate>
            <flux:button variant="ghost" icon="arrow-left" size="sm">All Posts</flux:button>
        </a>
    ...
    <div class="grid grid-cols-3 gap-6">
        ...
        <div class="col-span-2 space-y-5">
            ...
            <div>
                ...
                <flux:input wire:model.live="title" placeholder="Post title..." class="mt-1" />
            ...
            {{-- TipTap Editor --}}
            <div>
                <flux:label class="mb-1">Content</flux:label>
                <div
                ...
                >
                    ...
                    <div class="flex flex-wrap items-center gap-1 rounded-t-xl border border-b-0 border-zinc-200 bg-zinc-50 px-2 py-1.5">
                        <button type="button" @click="editor.chain().focus().toggleBold().run()"
                            class="px-2 py-1 text-sm font-bold rounded hover:bg-zinc-200 transition-colors"
                            :class="{ 'bg-zinc-200': editor?.isActive('bold') }">B</button>

                        <button type="button" @click="editor.chain().focus().toggleItalic().run()"
                            class="px-2 py-1 text-sm italic rounded hover:bg-zinc-200 transition-colors"
                            :class="{ 'bg-zinc-200': editor?.isActive('italic') }">I</button>

                        <button type="button" @click="editor.chain().focus().toggleHeading({ level: 2 }).run()"
                            class="px-2 py-1 text-sm font-semibold rounded hover:bg-zinc-200 transition-colors"
                        ...
                        <button type="button" @click="editor.chain().focus().toggleBulletList().run()"
                            class="px-2 py-1 text-sm rounded hover:bg-zinc-200 transition-colors"
                        ...
                        <button type="button" @click="insertImageUrl()"
                            class="px-2 py-1 text-sm rounded hover:bg-zinc-200 transition-colors">🖼 URL</button>

                        <label class="px-2 py-1 text-sm rounded hover:bg-zinc-200 transition-colors cursor-pointer">
                            📁 Upload
