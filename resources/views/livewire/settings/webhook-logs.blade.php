<div class="max-w-6xl mx-auto p-6 space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <flux:text class="mt-1">{{ __('Incoming webhook events from connected platforms.') }}</flux:text>
        </div>
    ...
    <div class="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
        <table class="w-full text-sm">
            <thead class="bg-zinc-50 dark:bg-zinc-800">
                <tr>
                    <th class="px-4 py-3 text-left font-medium text-zinc-500 dark:text-zinc-400">{{ __('Platform') }}</th>
                    <th class="px-4 py-3 text-left font-medium text-zinc-500 dark:text-zinc-400">{{ __('Event') }}</th>
                    <th class="px-4 py-3 text-left font-medium text-zinc-500 dark:text-zinc-400">{{ __('Status') }}</th>
                    <th class="px-4 py-3 text-left font-medium text-zinc-500 dark:text-zinc-400">{{ __('Received') }}</th>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse($logs as $log)
                    <tr class="bg-white hover:bg-zinc-50 dark:bg-zinc-900 dark:hover:bg-zinc-800">
                        ...
                        <td class="px-4 py-3">
                            @if($log->processed)
                                <flux:badge size="sm" color="green">{{ __('Processed') }}</flux:badge>
                            @elseif($log->error)
                        ...
                        <td class="px-4 py-3 text-right">
                            <flux:button size="sm" variant="ghost" wire:click="viewLog({{ $log->id }})">{{ __('View') }}</flux:button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-12 text-center text-zinc-400">{{ __('No webhook logs found.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <flux:modal :show="$viewingId !== null" @close="$wire.closeLog()" class="w-full max-w-3xl">
        @if($this->log)
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <flux:heading size="lg">{{ __('Webhook Event #') }}{{ $this->log->id }}</flux:heading>
                    <flux:badge color="{{ $this->log->processed ? 'green' : ($this->log->error ? 'red' : 'yellow') }}">
                        {{ $this->log->processed ? __('Processed') : ($this->log->error ? __('Failed') : __('Pending')) }}
                    </flux:badge>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        <span class="text-zinc-500 dark:text-zinc-400">{{ __('Event Type:') }}</span>
                    ...
                    @if($this->log->processed_at)
                        <div>
                            <span class="text-zinc-500 dark:text-zinc-400">{{ __('Processed At:') }}</span>
                ...
                <div>
                    <flux:label>{{ __('Payload') }}</flux:label>
                ...
                <div class="flex justify-end pt-2">
                    <flux:button variant="ghost" wire:click="closeLog">{{ __('Close') }}</flux:button>
                </div>
            </div>
        @endif
    </flux:modal>
