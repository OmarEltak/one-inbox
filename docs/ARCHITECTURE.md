...
- **Storage:** `MediaAsset` model + `MediaStorage` service. Per-team dedup by SHA-256 (composite unique index `(team_id, checksum_sha256)`). Local disk `storage/app/media/{team}/YYYY/MM/{ulid}.{ext}`, abstracted via `Storage::disk('media')` so S3 swap is a config change only.
- **Ingest:** each `PlatformInterface` implementer has `downloadInboundMedia(Page, mediaId, kind): MediaAsset`. Called from `ProcessIncomingMessage` per platform branch. `messages.media_asset_id` (nullable FK) links the message to its asset; existing `messages.media_url` and `messages.media_type` columns are populated with the signed URL + mime for legacy consumers.
- **Egress:** `SendPlatformMessage` inspects `$message->media_asset_id` and delegates to platform-specific `uploadOutboundMedia` + `sendMediaMessage`. Composer uploads via `POST /api/media/upload` returning an asset ULID; agent's Livewire component then dispatches `SendPlatformMessage` referencing that asset.
- **AI vision:** `VisionRouter::firstVisionCapableModel()` iterates the NaraRouter fallback chain (env `NARAROUTER_FALLBACK_MODELS`), returns the first model in a hardcoded `VISION_CAPABLE` map (agnes-*, glm-5.3-flash-free, minimax-m3-free, mistral-medium-3-5, stepfun-3.7-flash). `DescribeImage` job POSTs a multipart image_url payload, caches the description on `media_assets.metadata.ai_description`, then dispatches `SendAiResponse`, which injects the description into `$triggerMessage->content` in-memory (never persisted) so the text-only chat provider can reason about it.
- **AI transcription:** `TranscriptionRouter` with `GroqDriver` (primary, Groq Whisper large-v3 free tier, ~1-2s) + `WhisperCppDriver` (fallback, self-hosted `medium` model, ~40s on 2 CPU cores). Cache-backed `CircuitBreaker` per driver: 3 failures in 60s → open 5 min; 429 → cool driver for `Retry-After` seconds. Registered as singleton in `AppServiceProvider::register`; Groq skipped if `GROQ_API_KEY` empty or `TRANSCRIPTION_GROQ_ENABLED=false`.
- **Queue isolation:** whisper.cpp is CPU-bound and pins a core for ~40s. `TranscribeAudio` job routes to a dedicated `transcription` queue served by `one-inbox-whisper.service` (systemd, single worker, `CPUQuota=80%`, `Nice=10`, `MemoryMax=3G`). The 4 existing `one-inbox-queue@{1..4}` workers do NOT subscribe to `transcription`, so bursts of voice notes cannot starve text/image traffic on `urgent`/`default`.
- **Per-team rate limit:** 5 in-flight transcription jobs per team via Cache-backed atomic counter (`transcribe:inflight:{teamId}`). 6th job calls `$this->release(3)` and re-enters the queue with 3s delay. Counter has a 300s safety expiry in case a worker crashes without decrementing.
- **Kill switches (env):** `MEDIA_INGEST_ENABLED`, `VISION_ENABLED`, `TRANSCRIPTION_ENABLED`, `TRANSCRIPTION_GROQ_ENABLED`. All default true; each disables its own subsystem without a code deploy.
- **Per-team opt-out:** `teams.audio_transcription_enabled` (default true). When false, audio comes through as `[voice note]` and the AI never sees it — human agent handles manually.

### Do NOT
- Register the `transcription` queue on the 4 default workers. It exists to isolate CPU-bound jobs — mixing it back in defeats the design.
- Persist the injected vision description to `messages.content` in `SendAiResponse` — the in-memory mutation is intentional. Persisting it means future retries would double-inject.
- Add a "Groq is down, using local, please wait" apology reply — providers must return `''` on non-quota failure per §12. Local whisper transparently takes over; the customer doesn't need to know.
- Cross-team media dedup. `MediaStorage::storeBytes` scopes checksum uniqueness by `team_id` — do not "optimize" it to global uniqueness, or cached AI descriptions would leak across tenants.

### Files
- `app/Services/Media/MediaStorage.php`, `app/Models/MediaAsset.php`
- `app/Services/Ai/VisionRouter.php`
- `app/Services/Ai/TranscriptionRouter.php` + `app/Services/Ai/Transcription/*`
- `app/Jobs/DescribeImage.php`, `app/Jobs/TranscribeAudio.php`, `app/Jobs/ConvertAudioToOgg.php`
- `app/Http/Controllers/MediaController.php`, `app/Http/Controllers/Api/MediaUploadController.php`
- `resources/views/components/inbox/{media-bubble,lightbox,voice-recorder}.blade.php`
- `scripts/vps/install-whisper-medium.sh`, `scripts/vps/one-inbox-whisper.service`
